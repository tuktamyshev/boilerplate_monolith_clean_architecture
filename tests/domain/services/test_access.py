...
def test_check_is_admin_allows_superuser(access_service: AccessService) -> None:
    user = UserEntityFactory(is_superuser=True)

    access_service.check_is_admin(user)


def test_check_is_admin_denies_non_superuser(access_service: AccessService) -> None:
